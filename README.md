...
Xcode 15's `.xcstrings` format is a significant improvement over legacy `.strings` files — keys and translations live in a single JSON file, missing translations are flagged at compile time. Migration from `.strings` is one-way and is worth doing from the start.
